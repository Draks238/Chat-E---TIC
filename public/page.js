...
const vm = new Vue ({
  ...
  async created () {
    this.addNotification('¡Bienvenidos! Generando un nuevo par de claves ahora.')
    ...
    this.addNotification(`Par de claves generado - ${this.getKeySnippet(this.originPublicKey)}`)
  ...
  methods: {
    ...
    setupSocketListeners () {
      ...
      this.socket.on('connect', () => {
        this.addNotification('Conectando al servidor.')
        this.joinRoom()
      })
      ...
      this.socket.on('NEW_CONNECTION', () => {
        this.addNotification('Otro usuario se unió a la sala.')
        this.sendPublicKey()
      })
      ...
      this.socket.on('ROOM_JOINED', (newRoom) => {
        ...
        this.addNotification(`Se ha unido a la sala - ${this.currentRoom}`)
        this.sendPublicKey()
      })
      ...
      this.socket.on('ROOM_FULL', () => {
        this.addNotification(`No se puede unir ${this.pendingRoom}, Sala Llena`)
      ...
      this.socket.on('INTRUSION_ATTEMPT', () => {
        this.addNotification('Un tercer usuario intentó unirse a la sala.')
      })
    },
